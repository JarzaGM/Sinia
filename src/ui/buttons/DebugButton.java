...
public class DebugButton extends OnOffButton {
  ...
  public void render(Graphics g) {
    ...
    g.drawString(off, x + sx / 4 - g.getFont().getWidth(off) / 2,
        y + 3 * sy / 4 - g.getFont().getHeight(off) / 2);

    g.setColor(Color.white);
    g.drawString(on, x + sx - sx / 4 - g.getFont().getWidth(on) / 2,
        y + 3 * sy / 4 - g.getFont().getHeight(on) / 2);
